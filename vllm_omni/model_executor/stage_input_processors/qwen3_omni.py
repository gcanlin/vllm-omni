...
def _compute_talker_prompt_ids_length(info):
    ...
    thinker_sequences = torch.tensor(info["thinker_sequences"]).unsqueeze(0).to(current_platform.device_type).long()  # [1, T]
    input_ids = torch.tensor(info["thinker_input_ids"]).unsqueeze(0).to(current_platform.device_type).long()  # [1, T]
...
def thinker2talker(
    ...
    for i, thinker_output in enumerate(thinker_outputs):
        ...
        thinker_embeddings = output.multimodal_output["0"].float().clone().detach().to(current_platform.device_type)

        thinker_hidden_states = output.multimodal_output["24"].float().clone().detach().to(current_platform.device_type)
        info = {
            ...
            "tts_bos_embed": output.multimodal_output.get("tts_bos_embed").float().clone().detach().to(current_platform.device_type),
            "tts_eos_embed": output.multimodal_output.get("tts_eos_embed").float().clone().detach().to(current_platform.device_type),
            "tts_pad_embed": output.multimodal_output.get("tts_pad_embed").float().clone().detach().to(current_platform.device_type),
        }
